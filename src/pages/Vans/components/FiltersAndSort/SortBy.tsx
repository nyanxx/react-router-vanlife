...
export default function SortBy({ setQueries }: SortByProps): JSX.Element {
  function handleOptionClick(
    ...
    if (sortQuery === "Price: High - Low")
      setQueries({ for: "sortby", sortby: "HL" });
    if (sortQuery === "Price: Low - High")
      setQueries({ for: "sortby", sortby: "LH" });
  }

  return (
    <DropdownMenu.Root>
      <DropdownMenu.Trigger
        className={`cursor-pointer bg-[#f7c59b] text-[#161616] font-bold py-1 md:py-2 px-[0.8rem] rounded-lg text-[0.9rem] transition hover:bg-[#ffb476]`}
      >
        Sort by
        <CaretDownIcon
      ...
      <DropdownMenu.Content
        ...
        className="overflow-hidden bg-white text-nowrap shadow-lg z-99 mt-2 rounded-md"
      >
        {/* <DropdownMenu.Item className="py-2 px-4 block hover:bg-button cursor-pointer">Rating</DropdownMenu.Item> */}
        <DropdownMenu.Item className="py-2 px-4 block hover:bg-button cursor-pointer">
          Price: High - Low
        </DropdownMenu.Item>
        <DropdownMenu.Item className="py-2 px-4 block hover:bg-button cursor-pointer">
          Price: Low - High
