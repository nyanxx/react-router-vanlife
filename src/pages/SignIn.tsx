...
export default function SignIn(): JSX.Element {
  ...
  async function handleSignIn(formData: FormData) {
    ...
    if (email && password) {
      ...
      if (!loginSuccess) console.error("login failed");
      if (loginSuccess) navigate("/user");
    }
  ...
  const { isPasswordVisible, inputType, togglePasswordVisibility } =
  ...
  return (
    <section className="flex justify-center items-center h-full">
      ...
      >
        <h2 ref={head} tabIndex={-1} className="text-lg focus:outline-none">
          SignIn to your account
        </h2>
        <label htmlFor="email" className="sr-only">
          Enter your signin email
        </label>
        <input
          id="email"
          name="email"
        ...
        <p className="text-gray-500 text-sm">
          ...
          <Link
            className="text-button [word-spacing:-0.07rem] hover:underline "
            to="/signup"
          >
            Create a new one
          </Link>
