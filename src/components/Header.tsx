...
import { MenuContent, MenuHamburger, MobileMenu } from "./MobileMenu";
...
export default function Header(): JSX.Element {
  ...
  return (
    <>
      <header className="px-12 bg-header flex justify-between items-center fixed w-full z-1000 h-16 shadow-[0_2px_3px_-1px_rgba(0,0,0,0.3)]">
        <Link to="/" aria-label="Go to home page">
          <h1 className="font-bold text-[1.5rem] uppercase text-3xl">
            #VanLife
          </h1>
        </Link>
        {/* Mobile Menu Slider */}
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                [
                  ...
                  isActive
                    ? "p-1 mr-8 text-[#161616] font-bold border-b-2 border-solid border-black"
                    : "",
                ]
                  .filter(Boolean)
                  .join(" ")
          ...
          {authStatus === "authenticated" ? (
            ...
            >
              <NavLink
                ...
                className={({ isActive }) =>
                  `p-1 ${isActive ? "text-button" : "text-[#161616]"}`
                }
                title="User Profile"
              >
                <svg
              ...
              {userMenuActive && (
                <div className="absolute top-10 bg-amber-100 flex flex-col justify-center items-center rounded-lg overflow-hidden">
                  <Link
                    to={"/user"}
                    className="hover:bg-amber-200 p-4 cursor-pointer"
                  >
                    Dashboard
                  </Link>
                  <button
                    onClick={() => logout()}
                    className="hover:bg-amber-200 p-4 w-full cursor-pointer"
                  >
                    Signout
                  </button>
                </div>
